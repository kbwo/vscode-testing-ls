...
export class TestingConfig {
  ...
  get clientOptions(): LanguageClientOptions {
    const options: LanguageClientOptions = {
      documentSelector: this.fileTypes.map(lang => ({ scheme: 'file', language: lang })),
      synchronize: {
        fileEvents: vscode.workspace.createFileSystemWatcher(`**/*.{${this.fileTypes.join(',')}}`)
      }
    };

    const traceServer = this.config.get<string>('trace.server', 'off');
    if (traceServer !== 'off') {
      options.traceOutputChannel = vscode.window.createOutputChannel('Testing Language Server Trace');
    }
